...
public final class JogoAcha5 {
    ...
    public void iniciar(final Scanner scanner) {
        Console.println("Bem-vindo ao jogo Acha-5!");
        ...
        Console.println("Computador escolheu sua palavra secreta.");
        jogador1.setPalavraSecreta(obterPalavraSecretaJogador(scanner));
        Console.println("Que comece o jogo!");
        ...
        while (jogoAtivo) {
            if (vezDoJogador) {
                ...
                Console.printf("A palavra '%s' tem %d letra(s) correta(s)!%n", tentativa, acertos);
                if (acertos == TAMANHO_PALAVRA) {
                    Console.println("Parabéns! Você descobriu a palavra secreta do computador!");
                    jogoAtivo = false;
                }
            } else {
                final String tentativa = sortearPalavra();
                Console.printf("%nVez do computador! Ele tentou: '%s'%n", tentativa);
                jogador2.registrarTentativa(tentativa);
                final int acertos = contarLetrasCorretas(tentativa, jogador1.getPalavraSecreta());
                Console.printf("O computador encontrou %d letra(s) correta(s)!%n", acertos);
                if (acertos == TAMANHO_PALAVRA) {
                    Console.println("O computador descobriu sua palavra secreta! Você perdeu!");
                    jogoAtivo = false;
                }
            }
    ...
    private String obterPalavraSecretaJogador(final Scanner scanner) {
        while (true) {
            ...
            Console.println("Palavra inválida! Certifique-se de que possui exatamente 5 letras.");
        }
    }
    ...
    private String lerTentativaJogador(final Scanner scanner) {
        while (true) {
            ...
            Console.println("Palavra inválida! Tente novamente.");
        }
    }
