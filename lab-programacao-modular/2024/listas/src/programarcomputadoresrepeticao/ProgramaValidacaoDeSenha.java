...
public final class ProgramaValidacaoDeSenha {
    ...
    public static void main(final String[] args) {
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            Console.println("Bem-vindo ao sistema de autenticação.");
            ...
            for (int tentativa = 1; tentativa <= MAX_TENTATIVAS; tentativa++) {
                ...
                if (!validarTamanho(nome, MIN_CARACTERES, MAX_CARACTERES)) {
                    Console.println("Erro: O nome deve ter entre " + MIN_CARACTERES + " e " + MAX_CARACTERES + " caracteres.");
                    continue;
                }
                final String senha = EntradaUtil.lerLinha(scanner, "Digite sua senha: ");
                if (!validarTamanho(senha, MIN_CARACTERES, MAX_CARACTERES)) {
                    Console.println("Erro: A senha deve ter entre " + MIN_CARACTERES + " e " + MAX_CARACTERES + " caracteres.");
                    continue;
                }
                if (autenticador.validarCredenciais(nome, senha)) {
                    Console.println("Autenticação bem-sucedida. Bem-vindo, " + nome + "!");
                    return;
                }
                Console.println("Erro: Nome de usuário ou senha incorretos.");
                if (tentativa < MAX_TENTATIVAS) {
                    Console.println("Você ainda tem " + (MAX_TENTATIVAS - tentativa) + " tentativa(s).");
                }
            }
            Console.println("Número máximo de tentativas atingido. Acesso bloqueado.");
        }
    }
