...
public final class CaixaAutomatico {
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            do {
                ...
                switch (opcao) {
                    ...
                    case 0 -> Console.println("Encerrando o programa. Obrigado!");
                    default -> Console.println("Opção inválida. Por favor, tente novamente.");
                }
            } while (opcao != 0);
        }
    }
    ...
    private static void exibirMenu() {
        Console.println("\n=== Caixa Automático ===");
        Console.println("1 - Realizar retirada");
        Console.println("2 - Adicionar notas ao caixa");
        Console.println("3 - Apresentar relatório");
    ...
    private static void realizarRetirada(final Scanner scanner, final Caixa caixa) {
        ...
        if (valor % MULTIPLO_NOTA != 0) {
            Console.println("Erro: O valor deve ser múltiplo de " + MULTIPLO_NOTA + ".");
            return;
        }
        final Optional<Caixa.RetiradaResultado> resultado = caixa.realizarRetirada(valor);
        if (resultado.isEmpty()) {
            Console.println("Erro: Não há notas suficientes para realizar a retirada.");
            return;
        }
        final Caixa.RetiradaResultado r = resultado.get();
        Console.println("Retirada realizada com sucesso!");
        Console.printf("Notas de 50: %d%n", r.qtd50());
        Console.printf("Notas de 10: %d%n", r.qtd10());
    }
    ...
    private static void adicionarNotas(final Scanner scanner, final Caixa caixa) {
        ...
        if (adiciona50 < 0 || adiciona10 < 0) {
            Console.println("Erro: A quantidade de notas não pode ser negativa.");
            return;
        }
        caixa.adicionarNotas(adiciona50, adiciona10);
        Console.println("Notas adicionadas com sucesso!");
    }

    private static void apresentarRelatorio(final Caixa caixa) {
        Console.println("\n=== Relatório do Caixa ===");
        Console.printf("Notas de 50 disponíveis: %d%n", caixa.getNotasDe50());
        Console.printf("Notas de 10 disponíveis: %d%n", caixa.getNotasDe10());
        Console.printf("Valor total disponível: R$ %d%n", caixa.getValorTotalDisponivel());
        Console.printf("Valor total de retiradas efetuadas: R$ %d%n", caixa.getTotalRetiradas());
    }
}
