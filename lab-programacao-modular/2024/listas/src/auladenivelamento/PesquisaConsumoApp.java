...
public final class PesquisaConsumoApp {
    ...
    private static double lerPrecoKWh(final Scanner scanner) {
        while (true) {
            Console.print("Digite o preço do kWh: ");
            final Optional<Double> valor = lerDouble(scanner);
            if (valor.isPresent() && valor.get() > 0) {
                return valor.get();
            }
            if (valor.isPresent()) {
                Console.println("O preço deve ser positivo. Tente novamente.");
            } else {
                Console.println("Valor inválido. Digite um número.");
            }
        }
    }
    ...
    private static void lerConsumidores(final Scanner scanner, final PesquisaConsumo pesquisa) {
        while (true) {
            ...
            if (numeroOpt.isEmpty()) {
                Console.println("Valor inválido. Digite um número inteiro.");
                continue;
            }
            ...
            if (consumoKWh < 0) {
                Console.println("Consumo não pode ser negativo. Tente novamente.");
                continue;
            }

            Console.print("Digite o tipo do consumidor (residencial, comercial ou industrial): ");
            final String tipoStr = scanner.next().trim();
            final Optional<TipoConsumidor> tipoOpt = TipoConsumidor.fromString(tipoStr);
            if (tipoOpt.isEmpty()) {
                Console.println("Tipo inválido! Use: residencial, comercial ou industrial.");
                continue;
            }
            ...
            try {
                ...
                Console.println("Erro: " + e.getMessage() + " Tente novamente.");
            }
        }
    }
