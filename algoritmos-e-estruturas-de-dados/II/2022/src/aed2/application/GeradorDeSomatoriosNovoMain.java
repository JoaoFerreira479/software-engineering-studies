...
public final class GeradorDeSomatoriosNovoMain {
    ...
    private GeradorDeSomatoriosNovoMain() {}
...
final class SistemaSomatorio {
    ...
    public void iniciar() {
        Console.println("=== Sistema de Análise e Gerador de Somatórios ===\n");

        try (Scanner scanner = new Scanner(System.in)) {
            while (true) {
                Console.println("Digite o somatório para analisar ou 'FIM' para sair:\n");
                String entrada = scanner.nextLine();
                if (entrada == null) entrada = "";
                entrada = entrada.trim();

                if (entrada.equalsIgnoreCase(GeradorDeSomatoriosNovoMain.COMANDO_FIM)) {
                    Console.println("Saindo do programa. Até logo!");
                    break;
                }
                ...
                try {
                    ...
                    Console.println("Erro inesperado: " + e.getMessage());
                }
            }
        }
    }
    ...
    private void analisarSomatorio(String entrada) {
        ...
        if (normalizada.matches(GeradorDeSomatoriosNovoMain.REGEX_INTEIROS)) {
            analisarInteiros(normalizada);
        } else if (normalizada.matches(GeradorDeSomatoriosNovoMain.REGEX_FRACOES)) {
            analisarFracoes(normalizada);
        } else if (normalizada.matches(GeradorDeSomatoriosNovoMain.REGEX_POTENCIAS)) {
    ...
    private void analisarInteiros(String entrada) {
        ...
        int[] valores = GeradorDeSomatoriosNovoMain.parseInteiros(partes);
        ...
        if (isPA) {
            ...
            Console.println("Representação do somatório: " + representacao);
            Console.println("Propriedade: Progressão Aritmética (PA)");
        } else if (isPG) {
            Console.println("Representação do somatório: ∑ (a * r^i) (i de 0 até " + (valores.length - 1) + ")");
            Console.println("Propriedade: Progressão Geométrica (PG)");
        } else {
            Console.println("Representação do somatório: Não segue PA ou PG");
            Console.println("Propriedade: Nenhuma");
        }

        int soma = GeradorDeSomatoriosNovoMain.somaInteiros(valores);
    ...
    private void exibirResultados(String entrada, Object somaDecimal, String tipo, String somaFracao) {
        ...
        Console.println("Soma calculada (decimal): " + somaDecimal);
        Console.println("Soma calculada (fração): " + somaFracao + "\n");
    }
}
